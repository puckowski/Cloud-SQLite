...
const diffVDom = (vOldNode, vNewNode, viewModel = null) => {
    ...
    if (vOldNode &&
        ...
        if (vNewNode.tagName) {
            ...
            if (viewModel) {
            if (viewModel.slOnInit && !vOldNode.slOnInit && vOldNode.slUnboundOnInit !== viewModel.slOnInit) {
                vOldNode.slUnboundOnInit = viewModel.slOnInit;
                viewModel.slOnInit.bind(buildObj.model)();
                vNewMode.slOnInit = true;
            } else if (vOldNode.slUnboundOnInit !== undefined && !viewModel.slOnInit) {
                vOldNode.slOnInit = false;
            } else if (viewModel.slOnInit && vOldNode && !vOldNode.slOnInit) {
                if (vOldNode.slUnboundOnInit === viewModel.slOnInit) viewModel.slOnInit.bind(buildObj.model)();
                vOldNode.slUnboundOnInit = viewModel.slOnInit;
                vNewMode.slOnInit = true;
            } else if (viewModel.slOnInit && vOldNode.slOnInit && vOldNode.slUnboundOnInit === undefined) {
                vOldNode.slUnboundOnInit = viewModel.slOnInit;
            }

            if (viewModel.slAfterInit && !vOldNode.slAfterInit && vOldNode.slUnboundAfterInit !== buildObj.slUnboundAfterInit) {
                vOldNode.slAfterInit = true;
                vOldNode.slUnboundAfterInit = buildObj.slUnboundAfterInit;
                s._afterInitArr.push(viewModel.slAfterInit);
            } else if (vOldNode.slUnboundAfterInit !== undefined && !viewModel.slAfterInit) {
                vOldNode.slAfterInit = false;
            } else if (viewModel.slAfterInit && vOldNode && !vOldNode.slAfterInit) {
                vOldNode.slAfterInit = true;
                if (vOldNode.slUnboundAfterInit === buildObj.slUnboundAfterInit) s._afterInitArr.push(viewModel.slAfterInit);
                vOldNode.slUnboundAfterInit = buildObj.slUnboundAfterInit;
            } else if (viewModel.slAfterInit && vOldNode.slAfterInit && vOldNode.slUnboundAfterInit === undefined) {
                vOldNode.slUnboundAfterInit = buildObj.slUnboundAfterInit;
            }
        }
        } else {
